...
final public class AES256CBC {
    ...
    // MARK: - Cashdoc Encrypt
    // Api 전용
    public class func apiEncryptCashdoc(_ str: String) -> String? {
        let key = UserManager.shared.apiEncSecretKey
        let iv = UserManager.shared.apiEncIV
    ...
    public class func encryptCashdoc(_ str: String) -> String? {
        let key = UserManager.shared.encSecretKey
        let iv = UserManager.shared.encIV
    ...
    // MARK: - Cashdoc Decrypt
    
    public class func decryptCashdoc(_ encryptedString: String, user: User? = nil) -> String? {
        var key = UserManager.shared.encSecretKey
        var iv = UserManager.shared.encIV
        if let user = user {
            key = user.code.toKey ?? ""
            if let createdAt = user.createdAt {
                iv = String(createdAt).toIV ?? ""
            }
        }
        
        guard !encryptedString.isEmpty,
            key.length == 32,
            iv.length == 16 else {
                return nil
        }
        ...
        // remove IV
        guard let decryptedString = try? aesDecrypt(encryptedString, key: key, iv: iv) else {
