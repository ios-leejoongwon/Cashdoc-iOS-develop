...
open class ZGTooltipView: UIView {
    ...
    public required init(direction: Direction, customView: UIView, originView: UIView, removeOnDismiss: Bool = true) {
        ...
        if removeOnDismiss {
            dismissTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(ZGTooltipView.timerDidFire), userInfo: nil, repeats: false)
        }
    ...
    fileprivate func setup() {
        ...
        backgroundColor = UIColor(red: 255/255.0, green: 210/255.0, blue: 0/255.0, alpha: 1.0)
    ...
    public func displayTooltip(completion: (() -> Void)? = nil) {
        ...
        self.layoutMargins = UIEdgeInsets(top: 8, left: 13, bottom: 8, right: 13)
