...
final class MonthPickerView: UIPickerView {
    ...
    func setComponent(year: String, month: String) {
        guard let year = Int(year), let month = Int(month) else { return }
        self.selectRow(year - 1900, inComponent: 0, animated: false)
        self.selectRow(month - 1, inComponent: 1, animated: false)
    }
}
