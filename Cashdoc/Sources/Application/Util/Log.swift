...
class Log {
    ...
    class func d(_ dict: [String: Any]) {
        ...
        for (key, value) in order {
            ...
            if let getString = value as? String {
                print(getString.prefix(8000)) // String형태일때 길면 4000자로 짤라주자
            } else {
                print(value)
            }
        }
    ...
    // bzjoowan's Log
    class func bz(_ any: Any, filename: String = #file, funcName: String = #function) {
        #if DEBUG
        print("[🐸bzjoowan][\(sourceFileName(filePath: filename))] \(funcName) -> \(any)")
        #endif
    }
    
    // osh's Log 개인취향존중 알아서 바꾸세여
    class func osh(_ any: Any, filename: String = #file, funcName: String = #function) {
        #if DEBUG
    ...
    // al's Log
    class func al(_ any: Any, filename: String = #file, funcName: String = #function) {
        #if DEBUG
        print("[🙀al][\(sourceFileName(filePath: filename))] \(funcName) -> \(any)")
        #endif
    }
